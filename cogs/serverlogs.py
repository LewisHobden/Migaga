...
from model.model import ServerLogChannel
...
async def _notify(embed: discord.Embed, guild: discord.Guild, event_group: EventGroup):
    logs_channels = ServerLogChannel.get_for_guild(guild, event_group)

    for channel in logs_channels:
        server_logs = discord.utils.get(guild.channels, id=channel.channel_id)
...
class ServerLogs(commands.Cog, name="Server Logs"):
    ...
    async def _on_message_delete(self, message: discord.Message):
        ...
        await _notify(e, message.channel.guild)
    ...
    async def _on_message_edit(self, message_before: discord.Message, message_after: discord.Message):
        ...
        await _notify(e, message_after.channel.guild)
    ...
    async def _log_channel(self, ctx: SlashContext, log_channel: TextChannel):
        db_channel = ServerLogChannel.add_for_channel(ctx.guild.id, log_channel.id)
